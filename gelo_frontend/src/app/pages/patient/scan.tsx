...
export function MedicalAssessment() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (filesArr.length > 0) {
        ...
        setIsBlurry(blurry);
        if (blurry) toast.warning("Poor Image Quality", "Image seems blurry. Please retake for better AI accuracy.");
      }
    }
  };
  ...
  const scanMutation = useMutation({
    ...
    onMutate: () => {
      ...
      setTimeout(() => setProcessStatus("analyzing"), 1500); // UI feel
    },
    onSettled: () => setProcessStatus("idle"),
  });
  ...
  const handleInitiateScan = async () => {
    ...
    if (uploadedFiles.length === 0) {
     ...
     formData.append("imageQuality", isBlurry ? "BLURRY" : "CLEAR");
    ...
    <Layout>
      <div className="max-w-4xl mx-auto space-y-6 pb-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-4 border-b border-slate-100">
         ...
         <div ref={errorRef}>
           {analysisError && (
             <div className="p-4 bg-red-50 border border-red-100 text-red-600 rounded-2xl flex items-center gap-3 animate-in slide-in-from-top-2 mb-6">
            ...
            {uploadedFiles.length > 0 ? (
              <div className="space-y-6">
                ...
                <button
                  ...
                  className="cursor-pointer w-full py-4 bg-[#2a64ad] text-white font-black text-lg rounded-2xl hover:bg-[#1e4e8c] hover:shadow-xl transition-all flex items-center justify-center gap-3 disabled:opacity-50"
                >
