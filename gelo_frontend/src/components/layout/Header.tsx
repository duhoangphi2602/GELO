...
export function Header() {
  ...
  const getPageTitle = () => {
    switch (location.pathname) {
      ...
      case "/reports": return "Analytical Reports";
    ...
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 sticky top-0 z-30 shadow-sm">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-4">
          {/* Notification bell removed as requested */}          <div 
            onClick={() => navigate("/profile")}
            className="flex items-center gap-3 pl-4 border-l border-slate-200 cursor-pointer group"
          >
            <div className="w-9 h-9 rounded-full bg-blue-100 flex items-center justify-center text-[#2a64ad] font-bold text-sm border border-blue-200 group-hover:bg-[#2a64ad] group-hover:text-white transition-all">
              {initials}
            </div>
            <div className="flex flex-col hidden sm:flex">
              <span className="text-sm font-semibold text-slate-800 leading-none group-hover:text-[#2a64ad] transition-colors">{fullName}</span>
              <span className="text-[11px] text-slate-500 mt-1">Patient Account</span>
            </div>
          </div>
