...
FALLBACK_LABELS = {
    ...
    "2": {"id": 3, "code": "C43", "status": "DISEASE", "name": "Melanoma"},
...
class PredictorService:
    ...
    def predict(self, tensor_input: torch.Tensor) -> dict:
        ...
        # Normalize unexpected statuses to UNKNOWN
        if status != "DISEASE":
            logger.info(
                f"Label status '{status}' is not DISEASE. Normalizing prediction to UNKNOWN."
            )
            disease_id = 0
            code = "UNKNOWN"
            status = "UNKNOWN"
